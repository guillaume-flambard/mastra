...
String comparisons are exact and case-sensitive. The planner rejects a string where a number is required, including for score and `durationMs`, and accepts only ISO timestamp strings for timestamp predicates. A missing field satisfies neither positive nor ordered predicates, although the negative operators `ne` and `notIn` do match it. Combine a negative predicate with `exists` when the field must also be present.

### Filter by metadata

Use `metadata.<key>` paths for custom dimensions stored on the current root span. Metadata predicates support non-empty, top-level string values. Null, empty, numeric, boolean, object, and array values count as missing.
...
The key must name one top-level property. Empty keys and nested paths are rejected. Promoted metadata keys, such as `resourceId`, `threadId`, and `environment`, must use their canonical trace fields instead. Sensitive key names aren't queryable.
...
### Qualify conversations across traces

Set `group.by` to `['threadId']` and use `group.where` to qualify a conversation using evidence from its eligible traces. The top-level `timeRange` and `where` first select completed traces. `group.where` then evaluates only those traces within each non-null thread.

This query finds conversations where one eligible trace has a factuality score below `0.6` and one eligible trace has a clinician correction:
...
Every condition inside one `traces.some` applies to the same trace. Separate `traces.some` clauses may match the same trace or different traces in the conversation. `traces.none` requires that no eligible trace matches its complete nested predicate. Nested `spans`, `scores`, and `feedback` clauses retain their same-record binding.

`traces.some` and `traces.none` are available only inside `group.where`. A `group.where` condition requires `group.by: ['threadId']`. The API doesn't expose `groupBy`, top-level `having`, or `anyTrace` aliases.
