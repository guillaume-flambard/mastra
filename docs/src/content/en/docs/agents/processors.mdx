...
Mastra's background runtime remains responsible for execution, persistence, retries, result reconciliation, and continuation. The processor only retains caller-scoped notification access. It emits `work-completed` or `work-failed` after the authoritative tool result is reconciled.

Notifications are process-local and best-effort. If the originating run has ended, the task resumes on another process, or signal delivery fails, Mastra preserves the authoritative result without recreating the run or retrying the notification. Foreground calls don't emit background work notifications.
