...
describe('Postgres advanced trace query', () => {
  ...
  it('qualifies grouped threads with correlated eligible trace predicates', () => {
    ...
    expect(compiled.text).toContain('SELECT 1 FROM candidates r');
    expect(compiled.text).toContain('r."threadId" = g."threadId"');
    expect(compiled.text).toContain('SELECT 1 FROM current_scores s');
